...
func pubsocketdHandshake(config *websocket.Config, req *http.Request) (err error) {
	...
	if origin == "" {
		psdLogger.Printf("[%s][%s][handshake] missing origin", realIP, remoteAddr)
		return log.Printf("missing origin")
	}

	parsed, err := url.Parse(origin)

	if err != nil {
		psdLogger.Printf("[%s][%s][handshake] failed to parse origin, %v", realIP, remoteAddr, origin)
		return log.Printf("failed to parse origin")
	}
	...
	if parsed.String() != config.Origin.String() {
		psdLogger.Printf("[%s][%s][handshake] invalid origin, expected %v but got %v", realIP, remoteAddr, config.Origin, parsed)
		return log.Printf("invalid origin")
	}
...
func main() {
	...
	psdLogger := log.New(multi, "[pubsocketd] ", log.Ldate|log.Ltime|log.Lshortfile)

	if websocketAllowableOrigins == "" {
		err, _ := psdLogger.Fatalf("Missing allowable Origin (-ws-origin=http://example.com)")
		panic(err)
	}

	if !websocketAllowInsecure {
		if websocketAllowableOrigins == "" {
			err, _ := psdLogger.Printf("Missing allowable Origin (-ws-origin=http://example.com)")
			panic(err)
		}
		...
		if count > 1 {
			err, _ := psdLogger.Fatalf("Only one origin server is supported at the moment")
			panic(err)
		}
		...
		for _, test := range allowed {
			...
			if err != nil {
				err, _ := psdLogger.Printf("Invalid Origin parameter: %v, %v", test, err)
				panic(err)
			}
	...
	if websocketAllowInsecure {
		psdLogger.Printf("[init] listening for websocket requests on " + websocketEndpoint + websocketRoute + ", in INSECURE MODE which is not advised for production use")

	} else {
		psdLogger.Printf("[init] listening for websocket requests on " + websocketEndpoint + websocketRoute + ", from " + websocketAllowableOrigins)
	}

	psdLogger.Printf("[init] listening for pubsub messages from " + redisEndpoint + " sent to the " + redisChannel + " channel")

	if err := http.ListenAndServe(websocketEndpoint, nil); err != nil {
		err, _ := psdLogger.Fatalf("Failed to start websocket server, because %v", err)
	}
}
